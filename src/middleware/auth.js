...
//JWT is for restore password to mail
...
export const adminAccess = (req, res, next) => {
    ...
    try {
        ...
        req.logger.error(`Middleware auth.js adminAccess ${error.message}, ${error.code}`);
...
export const userAccess = (req, res, next) => {
    try {
        ...
        req.logger.error(`Middleware auth.js userAccess ${error.message}, ${error.code}`);
...
export const premiumAccess = (req, res, next) => {
    ...
    try {
        ...
        req.logger.error(`Middleware auth.js premiumAccess ${error.message}, ${error.code}`);
...
export const publicAccess = (req, res, next) => {
    ...
    try {
        ...
        req.logger.error(`Middleware auth.js publicAccess1 ${error.message}, ${error.code}`);
        res.status(500).send({ status: "error", error: `${error.name}: ${error.cause},${error.message},${error.code}` });
    }
};
export const privateAccess = (req, res, next) => {
    ...
    try {
        ...
        req.logger.error(`Middleware auth.js privateAccess ${error.message}, ${error.code}`);
        res.status(500).send({ status: "error", error: `${error.name}: ${error.cause},${error.message},${error.code}` });
    }
};
